...
class VacDB{
    ...
    //Returns an integer with how many indexes to add to the base index for each method of probing
    unsigned int probeStep(string patientName, int i, prob_t probType) const;
    
    //This function just sets the data to the new Patients data, used to make code look cleaner
    void assignPatient(Patient newPatient, Patient* oldPatient);

    //This checks if a rehash is needed and begins it if so
    void checkRehash();

    //This preforms an incremental rehash, returns true if the rehash is completed, and updates the new probtype
    void rehash();

    //Sets all the old values to be the current values
    void makeCurrentOld();

    //This function will insert the a node on the new hashmap from the old one.
    void transferPatient(Patient toTransfer);
